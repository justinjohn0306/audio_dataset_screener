...
namespace audio_dataset_screener
{
    ...
    {
        private int current_playing; //Record the current playback item number
        private int sortColumn = -1;//Click on the table header to sort ascending or descending
        ...
        private void frmMain_Load(object sender, EventArgs e)
        {
            ...
            playbackProgress = new PlaybackProgress(wmp);// Play progress class
        }
...
;        }
        ...
        private void frmMain_KeyDown(object sender, KeyEventArgs e) //define form hotkeys
        {
            if (grpPlaycontrol.Enabled & grpAction.Enabled)
            {
                ...
                e.Handled = true;
                e.SuppressKeyPress = true;//Prevent Windows default beep
            }
        ...
        {
            foreach (ListViewItem item in lvFileList.Items)// Determine if there are any unapplied actions
            {
                if(item.SubItems[1].Text != string.Empty)
                {
                    if (MessageBox.Show("There are still unapplied filter actions, do you really want to exit the program?", "Filter action not applied", MessageBoxButtons.OKCancel) != DialogResult.OK)
                    {
                    ...
                    {
                        return;// Directly end the FormClosing event method and close the window
                    }
                }
        ...
        {
            System.Diagnostics.Process.Start("https://github.com/justinjohn0306/audio_dataset_screener");
        }
    }
}
